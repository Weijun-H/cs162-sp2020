...
void serve_file(int fd, char *path) {
  ...
  snprintf(buf2, 100, "Conten_Legnth:%ld\r\n",rd);
  write(fd,buf2,100);
  http_end_headers(fd);
...
void serve_directory(int fd, char *path) {
  ...
  closedir(dirp);
    if(exit==1){
      ...
      puts(buf);
      int fd_server = open(buf, O_RDONLY);
      char buf1[10000];
      ssize_t rd = read(fd_server, buf1, 10000);
      write(fd,buf1,rd);
    }else{
      ...
      while((directory=readdir(dirp))!=NULL){
	char buf [100];
	http_format_href(buf, path, directory->d_name);
	write(fd,buf,100);
      }
    }
    return;
...
void *handle_clients(void *void_request_handler) {
  ...
  pthread_t thread;
  int client_fd;
  while((client_fd = wq_pop(&work_queue))>=0){
    pthread_create(&thread, NULL, request_handler, client_fd);
...
void serve_forever(int *socket_number, void (*request_handler)(int)) {
  ...
  *socket_number = socket(PF_INET, SOCK_STREAM, 0);
  ...
  while (1) {
    ...
    /* PART 6 BEGIN */
    pthread_t thread_1;
    pthread_create(&thread_1, NULL, request_handler, client_socket_number);
    pthread_join(thread_1,NULL);
    close(client_socket_number);
